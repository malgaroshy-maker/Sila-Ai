...
export default function DashboardInteractive({ initialJobs, initialResults, t, locale }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-[#0EA5E9]/30">
        ...
        <div className={`grid grid-cols-1 ${view === 'insights' ? '' : 'lg:grid-cols-3'} gap-6`}>
          ...
          <div className={view === 'insights' ? 'lg:col-span-3' : 'lg:col-span-2'}>
            <div className="bg-[#0F172A] rounded-2xl border border-[#1E293B] overflow-hidden">
              ...
              {view === 'list' ? (
                <>
                  <div className="px-5 py-2 border-b border-[#1E293B] flex items-center gap-3">
                    <input 
                      type="checkbox" 
                      className="w-4 h-4 rounded border-slate-600 text-[#0EA5E9] focus:ring-[#0EA5E9] bg-slate-800"
                      checked={filteredResults.length > 0 && selectedCandidateIds.size === filteredResults.length}
                      onChange={(e) => handleSelectAllCandidates(e.target.checked, filteredResults.map(r => r.candidate_id))}
                    />
                  ...
                  <div className="p-4 space-y-4 max-h-[700px] overflow-y-auto">
                    ...
                    filteredResults.map((res) => (
                      ...
                      >
                        ...
                        <div className="absolute top-4 start-4 z-20" onClick={(e) => e.stopPropagation()}>
                          <input 
                            type="checkbox" 
                            className="w-5 h-5 rounded border-slate-600 text-[#0EA5E9] focus:ring-[#0EA5E9] bg-slate-800"
                            checked={selectedCandidateIds.has(res.candidate_id)}
                            onChange={(e) => handleSelectCandidate(res.candidate_id, e.target.checked)}
      ...
      {selectedCandidateIds.size > 0 && (
        <div className="fixed bottom-6 start-1/2 -translate-x-1/2 rtl:translate-x-1/2 z-40 animate-in slide-in-from-bottom-10 fade-in duration-300">
          <div className="bg-[#0F172A] border border-[#1E293B] shadow-2xl shadow-blue-500/20 rounded-full px-6 py-3 flex items-center gap-4">
            <span className="text-slate-300 font-bold text-sm bg-[#1E293B] px-3 py-1 rounded-full whitespace-nowrap">
              {selectedCandidateIds.size} {t.selected || 'selected'}
            </span>
            <div className="w-px h-6 bg-[#1E293B]" />
            <button onClick={handleBulkDownload} disabled={isBulkActionLoading} className="text-sm font-bold text-white bg-[#0EA5E9] hover:bg-[#0284C7] px-4 py-2 rounded-full transition-colors flex items-center gap-2 disabled:opacity-50 whitespace-nowrap">
              {isBulkActionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
              {t.download_cvs || 'Download CVs'}
            </button>
            <button onClick={handleBulkDelete} disabled={isBulkActionLoading} className="text-sm font-bold text-rose-400 hover:text-white hover:bg-rose-500 px-4 py-2 rounded-full transition-colors flex items-center gap-2 disabled:opacity-50 whitespace-nowrap">
