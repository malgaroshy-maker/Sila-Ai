...
export default function WhatsAppResults({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#0F172A] rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-[#1E293B] shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-[#1E293B] flex justify-between items-center sticky top-0 bg-[#0F172A] z-10">
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <span className="text-slate-400 text-sm font-normal">— {candidateName}</span>
          </h2>
        ...
        <div className="p-6 space-y-6">
          {/* Score Card */}
          <div className={`rounded-2xl p-5 border ${scoreBg} flex items-center gap-4`}>
            <div className={`text-4xl font-black ${scoreColor}`}>{score}</div>
            <div>
              <div className={`text-lg font-bold ${scoreColor}`}>{verdictLabel}</div>
              <div className="text-xs text-slate-400 mt-0.5">{t.whatsapp_verdict_genuine ? '' : 'Authenticity Score'} /100</div>
            </div>
          </div>

          {/* AI Summary */}
          {session.summary && (
            <div className="bg-[#020617] rounded-xl p-4 border border-[#1E293B]">
              <h3 className="text-xs font-bold text-[#0EA5E9] uppercase tracking-widest mb-2 flex items-center gap-2">
          ...
          {session.red_flags && session.red_flags.length > 0 && (
            <div className="space-y-2">
              ...
              {session.red_flags.map((flag, i) => (
                <div key={i} className={`rounded-lg px-3 py-2 text-xs border ${severityColor(flag.severity)}`}>
                  <span className="font-bold uppercase mr-2">[{flag.severity}]</span>
                  {flag.reason}
                </div>
              ))}
            </div>
          ...
          {questions && questions.length > 0 && (
            <div className="space-y-3">
              ...
              {questions.map((q) => (
                <div key={q.question_number} className="bg-[#020617] rounded-xl p-4 border border-[#1E293B] space-y-3">
                  <div>
                  ...
                  <div className="ps-3 border-s-2 border-[#1E293B]">
                    <p className="text-sm text-slate-300">{q.answer_text || <span className="text-slate-600 italic">{'(No answer)'}</span>}</p>
                  </div>
        ...
        <div className="p-5 border-t border-white/5 flex justify-end">
          <button onClick={onClose} className="bg-white/5 hover:bg-white/10 border border-white/5 text-slate-300 font-bold py-2.5 px-5 rounded-xl cursor-pointer transition-all">
            {t.close || 'Close'}
          </button>
        </div>
      </div>
