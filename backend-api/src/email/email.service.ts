...
export class EmailService {
  ...
  constructor(private supabaseService: SupabaseService) {
    ...
    const googleRedirectUri =
      ...
      `${backendUrl}/email/auth/google/callback`;
    this.msRedirectUri =
      process.env.MS_REDIRECT_URI ||
      `${backendUrl}/email/auth/microsoft/callback`;

    this.logger.log(
      `Email Service initialized with Google Redirect: ${googleRedirectUri}`,
    );
  ...
  getGoogleAuthUrl(userEmail?: string, locale: string = 'ar') {
    ...
    const state = Buffer.from(JSON.stringify({ userEmail, locale })).toString(
      'base64',
    );
      ...
      if (state) {
        try {
          ...
          if (decoded.userEmail) {
            ...
            this.logger.log(
              `Associating Gmail ${emailAddress} with recruiter ${recruiterEmail}`,
            );
          }
          if (decoded.locale) {
      ...
      if (state) {
        try {
          ...
          if (decoded.userEmail) {
            recruiterEmail = decoded.userEmail;
            this.logger.log(
              `Associating Outlook ${emailAddress} with recruiter ${recruiterEmail}`,
            );
          }
          if (decoded.locale) {
            locale = decoded.locale;
