...
export class AiController {
  ...
  async getUsageLogs(@Headers('x-user-email') userEmail: string) {
    ...
    if (error) {
      ...
      return {
        logs: [],
        summary: { total_input: 0, total_output: 0, total_cost: 0 },
      };
    }
    ...
    const liveStatus = await this.aiService.getLiveQuota(
      settings.apiKey,
      settings.model,
    );
    ...
    const remaining =
      liveStatus?.last_seen_remaining !== null &&
      liveStatus?.last_seen_remaining !== undefined
