...
export class CandidatesController {
  ...
  async uploadCV(
    ...
    if (!jobId || !name)
      throw new BadRequestException('jobId and candidate name are required');

    return this.candidatesService.processCandidate(
      userEmail,
      jobId,
      name,
      email || '',
  ...
  async updateStage(
    ...
    return this.candidatesService.updateApplicationStage(
      userEmail,
      applicationId,
      stage,
    );
  ...
  async downloadCV(
    ...
    this.requireEmail(userEmail);
    const result = await this.candidatesService.downloadCV(
      userEmail,
      candidateId,
    );
